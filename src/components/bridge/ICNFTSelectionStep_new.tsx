...
import { Check, Image, AlertCircle, Loader2 } from 'lucide-react';
...
export interface ICNFTSelectionStepProps {
  ...
  /** Callback when a new canister is loaded for ICRC-99 checking */
...
export function ICNFTSelectionStep({
  ...
  const validateAndSetCanisterId = useCallback((canisterId: string) => {
    ...
    try {
      ...
      // Notify parent about canister change for ICRC-99 checking
      if (onCanisterChange) {
  ...
  // Get owned token IDs using the existing hook
  const { tokenIds, loading: tokenIdsLoading, error: tokenIdsError } = useNFTOwnerOf(
    activeCanisterId || 'aaaaa-aa', // Use activeCanisterId instead of sourceCanisterId
    userAccount || { owner: 'aaaaa-aa', subaccount: undefined }, // Dummy account if not available
  );
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Show results section only when we have NFTs loaded */}
      {!loading && !tokenIdsLoading && activeCanisterId && tokenIds.length > 0 && !error && (
        <>
...
function NFTCard({
  ...
  return (
    <div
      className={clsx(
        'relative bg-white rounded-lg border-2 transition-all duration-200 cursor-pointer group hover:shadow-lg',
        selected
          ...
          <div className="space-y-1">
            ...
            {nft.attributes.length > 2 && (
              <p className="text-xs text-gray-500">
                +{nft.attributes.length - 2} more attribute{nft.attributes.length - 2 !== 1 ? 's' : ''}
              </p>
            )}
          </div>
        )}
